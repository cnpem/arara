...
type ViewerStore = {
  ...
  beta: number;
  gamma: number;
  setLatticeParams: (params: { a: number; b: number; c: number; alpha: number; beta: number; gamma: number }) => void;

  spaceGroup: number;
  setSpaceGroup: (s: number) => void;
...
export const useViewerStore = create<ViewerStore>((set) => ({
  ...
  beta: 0.0,
  gamma: 0.0,
  setLatticeParams: ({ a, b, c, alpha, beta, gamma }) =>
    set({ a, b, c, alpha, beta, gamma }),

  spaceGroup: 0,
  setSpaceGroup: (n) => set({ spaceGroup: n }),
